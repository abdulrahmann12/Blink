...
public class UrlController {
    ...
    public ResponseEntity<Void> unlock(
        ...
        String originalUrl = urlService.unlockUrl(shortCode, request.getPassword(), httpRequest);
        return ResponseEntity.status(HttpStatus.FOUND)
                .location(URI.create(originalUrl))
                .build();
    }
    ...
    public ResponseEntity<BaseResponse> removePassword(@PathVariable UUID urlId) {
        ...
        return ResponseEntity.ok(new BaseResponse(Messages.PASSWORD_REMOVED, null));
    }
